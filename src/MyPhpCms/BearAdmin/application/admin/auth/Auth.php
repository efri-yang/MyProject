...
class Auth {
    ...
    protected function getAuthList($uid, $type) {
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) {
                ...
                //
                //$rule=Array ( [condition] => {score} > 50 and {score} <100 [name] => admin/index/index )
    ...
    private static function data_auth_sign($data) {
        //一个数组经过转化后 foo=bar&baz=boom&cow=milk&php=hypertext
        $code = http_build_query($data); //url编码并生成query字符串
        //sha 以后 就变成4d759639adf55f64691e8e66b288683c9de02dfc
        $sign = sha1($code); //生成签名
        return $sign;
    }
